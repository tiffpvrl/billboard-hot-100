...
That’s a huge drop. My guess? With streaming, viral trends, and global platforms like TikTok, a single breakout hit can dominate for months instead of passing the crown every week like songs often did in the past.
...
Even with only five years of data, the 2020s already show seven outlier songs, the most of any decade so far. This suggests the most concentrated chart dominance we’ve ever seen—exactly what you'd expect in a hyper-viral, algorithm-powered music world.
...
song_runs <- billboard_updated |>
  distinct(song, artist, weeks_at_number_one, year, decade, date) |>
  mutate(
    song_id = paste0(song, " — ", artist),
...
marathon_hits_only <- marathon_hits |>
  filter(type == "marathon")
```
...
Earlier decades topped out around 10 weeks. The 1990s broke through that barrier, and the 2010s–2020s shattered it.
...
And overall?
- There’s no single “magic month.” Dominance depends more on cultural moments and viral momentum than the calendar.
...
Earlier decades show a tightly linked cluster of danceability, energy, and loudness—a sign that upbeat songs tended to be uniformly bright, energetic, and dance-friendly. Think disco, classic pop, 80s synths, and power ballads. BPM and happiness also aligned, suggesting that “fast = cheerful” was a common formula for a hit. Acousticness sat far from the energy cluster—acoustic songs rarely overlapped with dance-heavy tracks.

This reflects a time when genres were more distinct, and production styles followed clearer rules.

Post 2000s, danceability no longer correlates with acousticness—modern songs blend “dance-friendly” rhythms with acoustic textures (think Ed Sheeran or folk-pop crossovers). BPM and happiness no longer travel together; a fast song doesn’t have to feel bright or positive (hello, hyperpop sadness or moody EDM).
